...
const Resumen = ({ ingresos, egresos }) => {
  ...
  return (
    <div >
      <h2 >Resumen</h2>
      <ul>
        <li>Ingresos netos: <span>${totalIngresos.toFixed(2)}</span></li>
        <li>Gastos fijos: <span>${gastosFijos.toFixed(2)}</span></li>
        <li>Gastos variables: <span>${gastosVariables.toFixed(2)}</span></li>
        <li>Diferencia: <span>${diferencia.toFixed(2)}</span></li>
      </ul>
    </div>
  );
};
